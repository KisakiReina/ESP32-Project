...
本项目基于ESPIDF的HID-Device示例改造，项目默认使用ESP32C6开发板，理论支持ESPIDF支持的所有开发板（有蓝牙功能的）。\
当**已配对的蓝牙靠近**时驱动舵机开锁，仅使用 **BLE 扫描**而不连接，仅在第一次配对时需要连接目标设备，之后使用IRK地址解析记录目标RPA地址，使即使配对的目标设备蓝牙随机mac改变也能正确识别已配对设备并驱动舵机触发解锁。
