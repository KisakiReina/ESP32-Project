...
/* External BLE GAP event callback (e.g., for RSSI read complete forwarding) */
static esp_hid_gap_ble_ext_cb_t s_ext_ble_gap_cb = NULL;

/* Raw BLE scan result callback (called for every scan result, no UUID filter) */
static esp_hid_ble_scan_raw_cb_t s_scan_raw_cb = NULL;

/* Track whether BLE scanning is actively running */
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT: {
            /* Notify raw scan callback (if registered) before UUID filtering */
            if (s_scan_raw_cb) {
                s_scan_raw_cb(scan_result->scan_rst.bda,
...
static esp_ble_scan_params_t hid_scan_params = {
    ...
    .scan_interval          = 0x50,
    .scan_window            = 0x30,
    .scan_duplicate         = BLE_SCAN_DUPLICATE_DISABLE, /* must be disabled — we need every RSSI update */
};
